...
export function EditItemSheet({ open, onOpenChange, containerId, item }: EditItemSheetProps) {
  const { updateItem } = useContainer();
  ...
  useEffect(() => {
    if (open && item) {
      ...
      setImageDataUrl(item.imageUrl || null);
    } else {
  ...
  function onSubmit(values: z.infer<typeof formSchema>) {
    updateItem(containerId, item.id, {
      name: values.name,
